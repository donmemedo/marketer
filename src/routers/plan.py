...
from schemas import CostIn, MarketerInvitationOut, MarketerInvitationIn, MarketerIdpIdIn, FactorIn, ResponseOut
from tools import to_gregorian_, peek, get_marketer_name
...
@plan_router.get("/factor/", dependencies=[Depends(JWTBearer())])
async def cal_marketer_factor(request: Request, args: FactorIn = Depends(FactorIn)):
    """_summary_
    ...
    if args.tax == 0 and args.collateral == 0:
        ...
        final_fee -= final_fee * 0.15
    two_months_ago_coll = 0
    dd=args.year+ f"{int(args.month)-2:02}"+"Collateral"
    two_months_ago_coll = factor_coll.find_one({"FullName":marketer_fullname})[dd]
    #
    # try:
    #     factor_coll.insert_one(
    #         {"MarketerID": marketer_dict['IdpId'], "ThisMonth": jd.today().month,
    ...
    pdf_maker(shobe='تهران', name=marketer_fullname, doreh=from_date, total_fee=int(marketer_total.get("TotalFee")),
              pure_fee=int(pure_fee), marketer_fee=int(marketer_fee), tax=int(tax), colat2=int(two_months_ago_coll), colat=int(collateral),
              mandeh= 0, pardakhti=int(final_fee + float(two_months_ago_coll)), date=jd.now().strftime('%C'))
    return {
        "TotalFee": marketer_total.get("TotalFee"),
        "TotalPureVolume": marketer_total.get("TotalPureVolume"),
        "PureFee": pure_fee,
